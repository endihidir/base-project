...
namespace UnityBase.GridSystem
{
    public class HexWorldGrid<T> : WorldGrid<T> where T : struct, IGridNodeData
    {
        ...
        public override void RebuildMeshVisual(Mesh mesh)
        {
            ...
            MeshExtensions.CreateEmptyMeshArraysHex3D(cellCount, out var vertices, out var uv, out var triangles);
            ...
            for (int x = 0; x < Width; x++)
            {
                for (int y = 0; y < Height; y++)
                {
                    for (int z = 0; z < Depth; z++)
                    {
                        ...
                        MeshExtensions.AddToMeshArraysHex3D(vertices, uv, triangles, index, worldPos, size.x / SQRT3, size.y, Transform, !_isPointyTopped);
                    }
                }
            }
